...
const api = () => {
  ...
  const getGroups = async (req, res) => {
    ...
    try {
      const query = `select * from tidy_group where group_id=${groupId}`;
  ...
  const deleteTask = async (req, res) => {
    ...
    return res.status(200).json({ message: "The Task have been deleted!" });
  };
  ...
  const addNewGroup = async (req, res) => {
    ...
    if (emailExists.rows.length > 0) {
      ...
      // answering with the task id
      res.status(200).json({ groupId: result.rows[0].id });
    }
  };
  ...
  const updateGroup = async (req, res) => {
    ...
    res.status(202).send(`Group has been updated!`);
  };
